...
from qlora_repo.modules.struc_dropout import dropout_struc, reduce_LoRA_r
import warnings
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    checkpoint_dir: str = field(
        default=None,
        metadata={
            "help": "The checkpoint dir for logs and checkpoints. Default: output_dir"
        },
    )
    optim: str = field(
...
class SelfDefinedArguments:
    struc_dropout: Optional[bool] = field(default=False)
    LoRA_reduced_rank: Optional[int] = field(default=-1)
    filter_with_source_max_len: Optional[bool] = field(default=True)
...
OPEN_INSTRUCT_PROMPT_DICT = {  # this is derived from open-instruct repo
...
def format_open_instruct_messages(example):
    """This function is modified from open-instruct repo. used for dataset.map()
        Here we assume each example has a 'messages' field Each message is a dict with 'role' and 'content' fields.
        We concatenate all messages with the roles as delimiters and tokenize them together.
        In the messages field, there must be 2/3-entry (i.e. system(optional), user and assistant).
    Args:
    ...
    ), f"messages field doesn't meet the requirements(i.e. 2/3-entry: system(optional), user and assistant) . please check the dataset to make sure it is correct.\n This message has {len(messages)} entries, as shown below:\n {messages}"

    if len(messages) == 2:
        return {
            "input": OPEN_INSTRUCT_PROMPT_DICT["user_assistant"].format(
        ...
        return {
            "input": OPEN_INSTRUCT_PROMPT_DICT["system_user_assistant"].format(
                system=messages[0]["content"].strip(),
                user=messages[1]["content"].strip(),
            ),
            "output": messages[2]["content"].strip(),
        }
    else:
        raise ValueError(
            f"messages field doesn't meet the requirements(i.e. 2/3-entry: system(optional), user and assistant) . please check the dataset to make sure it is correct.\n This message has {len(messages)} entries, as shown below:\n {messages}"
...
def make_data_module(
    tokenizer: transformers.PreTrainedTokenizer, args, filter_with_source_max_len
) -> Dict:
    """
    ...
    def load_data(dataset):
        """This is modified from open-instruct repo."""

        return load_dataset("json", data_files={"train": dataset})

    def format_dataset(dataset):
        """This is modified from open-instruct repo."""
        assert (
            "messages" in dataset["train"].column_names
        ), "messages not in column names. please check whether this dataset is preprocessed by open-instruct repo correctly."
        dataset = dataset.map(format_open_instruct_messages)
    ...
    if filter_with_source_max_len:
        dataset = dataset.filter(
            lambda example: len(tokenizer(example["input"])["input_ids"])
            <= args.source_max_len
        )
...
def get_last_checkpoint(checkpoint_dir, enable_completed_check=True):
    if isdir(checkpoint_dir):
        is_completed = exists(join(checkpoint_dir, "completed"))
        if is_completed and enable_completed_check:
...
def format_mmlu_to_open_instruct(mmlu_dataset):
...
def train():
    ...
    ), "Please specify checkpoint_dir in eval_qlora.py"
    checkpoint_dir, completed_training = get_last_checkpoint(
        args.checkpoint_dir, enable_completed_check=False
    )
    if completed_training:
        warnings.warn("Detected that training was already completed!")

    model, tokenizer = get_accelerate_model(args, checkpoint_dir)
    if selfdefined_args.struc_dropout:
        # dropout_struc(model)
        warnings.warn(
            "structural dropout is invalid for evaluation, and will be disabled automatically!"
        )
    if selfdefined_args.LoRA_reduced_rank != -1:
        reduce_LoRA_r(model, rank=selfdefined_args.LoRA_reduced_rank)
    ...
    data_module = make_data_module(
        tokenizer=tokenizer,
        args=args,
        filter_with_source_max_len=selfdefined_args.filter_with_source_max_len,
    )
    ...
    if args.do_mmlu_eval:
        if args.mmlu_dataset == "mmlu-zs":
            mmlu_dataset_loaded = load_dataset(
                "json",
                data_files={
        ...
        mmlu_dataset_openinstruct = format_mmlu_to_open_instruct(mmlu_dataset_loaded)
        mmlu_dataset = mmlu_dataset_openinstruct[args.mmlu_split]
        ...
        class MMLUEvalCallback(transformers.TrainerCallback):
            def on_evaluate(self, args, state, control, model, **kwargs):
                ...
                results = {f"mmlu_{args.mmlu_split}_loss": loss_mmlu / len(data_loader)}
            ...
            def on_train_end(self, args, state, control, model, **kwargs):
                mmlu_split = "test"
                mmlu_test_dataset = mmlu_dataset_openinstruct[mmlu_split]
                ...
                for batch in tqdm(data_loader, total=len(data_loader)):
                    ...
                    loss_mmlu += loss.item()
                # Extract results by subject.
                results = {f"mmlu_{mmlu_split}_loss": loss_mmlu / len(data_loader)}
    ...
    if args.do_eval:
        ...
        key_prefix = "eval" + ("_" + args.mmlu_split if args.do_mmlu_eval else "")
        metrics = trainer.evaluate(metric_key_prefix=key_prefix)
        all_metrics.update(metrics)

        eval_acc = [i for i in trainer.state.log_history if "mmlu_test_accuracy" in i]
        if len(eval_acc) == 0:
            raise ValueError("mmlu_test_accuracy not found in log history")
        else:
            print("eval_log:", eval_acc)
            eval_acc = eval_acc[-1]
        metrics.update(eval_acc)
        trainer.log_metrics(key_prefix, metrics)
        trainer.save_metrics(key_prefix, metrics)
